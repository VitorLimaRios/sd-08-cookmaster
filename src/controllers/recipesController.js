...
const updateRecipe = async (req, res) => {
  try {
    ...
    const { name, ingredients, preparation } = req.body;
    const result = await recipesServices
      .updateRecipeServices({ name, ingredients, preparation, id});
    res.status(result.statusCode).json(result.json);
  } catch (error) {
    res.status(FIVE_HUNDRED).json({
      message: 'Erro',
...
const deleteRecipe = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await recipesServices.deleteRecipesServices(id);
    res.status(result.statusCode).json(result.json);
  } catch (error) {
    res.status(FIVE_HUNDRED).json({
      message: 'Erro',
...
const createImage = async (req, res) => {
  try {
    const { id } = req.params;
    const image = `localhost:3000/src/uploads/${req.image}`;
    const result = await recipesServices.addImageServices(id, image);
    res.status(result.statusCode).json(result.json);
  } catch (error) {
