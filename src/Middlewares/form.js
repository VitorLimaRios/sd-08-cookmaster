...
const { getRecipeById } = require('../services/recipeFunctions.service');
const { ObjectId } = require('mongodb');
const {
  StatusCodes: { BAD_REQUEST, CONFLICT, UNAUTHORIZED, NOT_FOUND },
...
exports.validateForm = (req, res, next) => {
  try {
    ...
    if (!name || !password) throw new Error('Invalid entries. Try again.');
    next();
  } catch (err) {
    res.status(BAD_REQUEST).json({ message: err.message });
  }
...
exports.validateId = (req, res, next) => {
  try {
    ...
    if (!isValid) throw new Error('recipe not found');
    next();
  } catch (err) {
...
exports.userAuth = async (req, res, next) => {
  try {
    const { userId } = await getRecipeById(req.params.id);
    userAuthorization(req.user, userId);
    next();
  } catch (err) {
